...
class TestHelper(unittest.TestCase):
	...
	def test_tokenize(self):
		...
		with tf.Graph().as_default():
			...
			# Check value types
			bad_value_table = [type(val) for val in token_dict.values() if not isinstance(val, str)]
			self.assertIsNotNone(bad_value_table)

			# Check for spaces
			key_has_space = [key for key in token_dict.keys() if ' ' in key]
			value_has_space = [value for value in token_dict.values() if ' ' in value]
	...
	def test_create_lookup_table(self):
		...
		with tf.Graph().as_default():
			...
			vocab_to_int, int_to_vocab = preprocess.create_lookup_tables(test_text)
			...
			missmatches = [word for word, id in vocab_to_int.items() if int_to_vocab[id] != word]
	...
	def test_get_inputs(self):
		...
		with tf.Graph().as_default():
			inputs, targets, learning_rate = script_generator.get_inputs()

			# Check type
			self.assertEqual(inputs.op.type, "Placeholder")
			self.assertEqual(targets.op.type, "Placeholder")
			self.assertEqual(learning_rate.op.type, "Placeholder")
			...
			lr_rank = 0 if learning_rate.get_shape() == None else len(lr_rank.get_shape())
	...
	def test_get_init_cell(self):
		...
		with tf.Graph().as_default():
			...
			test_rnn_size = 256
			cell, init_state = script_generator.get_init_cell(test_batch_size_ph, test_rnn_size)
	...
	def test_get_batches(self):
		...
		with tf.Graph().as_default():
			...
			batches = preprocess.get_batches(test_int_text, test_batch_size, test_seq_length)
	...
	def test_get_embed(self):
		...
		with tf.Graph().as_default():
			...
			embed = script_generator.get_embed(test_input_data, test_vocab_size, test_embed_dim)
	...
	def test_build_rnn(self):
		...
		with tf.Graph().as_default():
			...
			outputs, final_state = script_generator.build_rnn(test_cell, test_inputs)
			...
			# Check shape
			self.assertEqual(outputs.get_shape().as_list(), [None, None, test_rnn_size])
			self.assertEqual(final_state.get_shape().as_list(), [test_rnn_layer_size, 2, None, test_rnn_size])
	...
	def test_build_nn(self):
		...
		with tf.Graph().as_default():
			...
			logits, final_state = script_generator.build_nn(test_cell, test_rnn_size, test_input_data, test_vocab_size, test_embed_dim)
			...
			# Check shape
			self.assertEqual(logits.get_shape().as_list(), test_input_data_shape + [test_vocab_size])
			self.assertEqual(final_state.get_shape().as_list(), [test_rnn_layer_size, 2, None, test_rnn_size])
	...
	def test_get_tensors(self):
		...
		input_text, initial_state, final_state, probs = script_generator.get_tensors(test_graph)
	...
	def test_pick_word(self):
		...
		with test_graph.as_default():
			...
			test_int_to_vocab = {ii: word for ii, word in enumerate(['this', 'is', 'a', 'test'])}

			pred_word = script_generator.pick_word(test_probabilities, test_int_to_vocab)
